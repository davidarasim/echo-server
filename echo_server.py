...
def server(log_buffer=sys.stderr):
    ...
    print("[server] making a server on {0}:{1}".format(*address), file=log_buffer)
    ...
    try:
        ...
        while True:
            print('[server] waiting for a connection', file=log_buffer)
            ...
            try:
                print('[server] connection - {0}:{1}'.format(*addr), file=log_buffer)
                ...
                while True:
                    ...
                    data = conn.recv(16)
                    print('[server] received "{0}"'.format(data.decode('utf8')))
                    ...
                    if data:
                        conn.sendall(data)
                        print('[server] sent "{0}"'.format(data.decode('utf8')))
                ...
                # sock.close() DKA
                print('[server] echo complete, client connection closed', file=log_buffer)
        ...
        sock.close()
        print('[server] quitting echo server', file=log_buffer)
